...
import { IoLogOutOutline, IoHomeOutline, IoBookOutline, IoCreateOutline } from 'react-icons/io5';
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col">
        <div className="p-6 border-b border-slate-800">
          <Link to={ROUTES.HOME} className="font-heading font-bold text-xl tracking-tight">
            LMS<span className="text-primary-400">Instructor</span>
          </Link>
        </div>
        <nav className="flex-1 p-4 space-y-2">
          <Link to={ROUTES.INSTRUCTOR_DASHBOARD} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-800 text-sm font-medium transition-all">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-100 flex items-center justify-between px-8">
          <h2 className="text-lg font-heading font-semibold text-slate-800">Instructor Portal</h2>
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium text-slate-600">Instructor Mode: {user?.name}</span>
            <div className="h-8 w-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-heading font-semibold text-sm">
